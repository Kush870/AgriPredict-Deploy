...
def load_or_create_models():
    """Load models and metrics from disk, or train fresh ones"""
    global pipeline, model_metrics, xgb_pipeline, xgb_metrics, tuned_pipeline, tuned_metrics, comparison_metrics, tuning_results
    
    print("Attempting to load saved models...")
    
    try:
        if os.path.exists(MODEL_PATH) and os.path.exists(XGB_MODEL_PATH) and os.path.exists(TUNED_MODEL_PATH):
            ...
            if os.path.exists(metrics_path):
                with open(metrics_path, 'r') as f:
                    ...
                    xgb_metrics = all_metrics.get('xgb_metrics', {})
                    tuned_metrics = all_metrics.get('tuned_metrics', {})
                    comparison_metrics = all_metrics.get('comparison_metrics', {})
                    tuning_results = all_metrics.get('tuning_results', {})
            else:
                compute_model_metrics()
            
            print("Models and metrics loaded successfully from disk.")
            return
...
def train_models(dataset_path=None, tune_rf=False):
    ...
    try:
        ...
        # Added max_depth=15 to dramatically reduce the model size (from ~728MB to ~15MB)
        rf_model = RandomForestRegressor(n_estimators=10, max_depth=15, random_state=seed, n_jobs=-1)
